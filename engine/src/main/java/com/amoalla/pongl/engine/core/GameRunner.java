...
import com.amoalla.pongl.engine.ecs.components.Background;
...
public class GameRunner implements AutoCloseable {
    ...
    private final ScriptExecutionSystem scriptRunner;
    ...
    private final Physics2DSystem physics;
    ...
    public GameRunner(GameConfig config) {
        ...
        physics = new Physics2DSystem(ecs);
        scriptRunner = new ScriptExecutionSystem(ecs);
        context = new Context(window, ecs, physics);
    }
    ...
    public void run(Game game) {
        ...
        tickScheduler.schedule(scriptRunner);
...
//            physicsRenderer.render(physics.world(), renderer);
    ...
    private void render() {
        // Todo solve with layers not by adding a background component
        var background = ecs.findCompositionsWith(SpriteComponent.class, Transform2DComponent.class, Background.class);
        for (var composition : background) {
        ...
        var foreground = ecs.findCompositionsWith(SpriteComponent.class, Transform2DComponent.class)
                .without(Background.class);
        for (var composition : foreground) {
            SpriteComponent sprite = composition.comp1();
            Transform2DComponent transform = composition.comp2();
            if (sprite.texture() == null) {
                renderer.drawQuad(transform.translation(), transform.scale(), transform.rotationAngle(), transform.rotationCenter(), sprite.tint());
