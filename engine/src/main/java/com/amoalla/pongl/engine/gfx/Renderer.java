...
//        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    ...
    public void drawPoint(float x, float y, Color color) {
        ...
        // Do not create instances of Vector2 everytime
        Vector4f size4f = new Vector4f(5, 5, 0, 0).mul(projection);
        Vector2f size = new Vector2f(size4f.x, size4f.y);
        drawQuad(new Vector2f(x, y), size, color);
    }
